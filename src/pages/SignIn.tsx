...
export default function SignIn({ onNavigate }: SignInProps) {
  ...
  const handleGoogleLogin = async () => {
    // Mock Google Login
    const success = await login('google-user@gmail.com', 'google-auth-token');
    if (success) {
      onNavigate('home');
    }
  };
  ...
  const handleGithubLogin = async () => {
    // Mock GitHub Login
    const success = await login('github-user@github.com', 'github-auth-token');
    if (success) {
      onNavigate('home');
    }
  };
  ...
  return (
    <div className="w-full min-h-[80vh] flex items-center justify-center px-4 relative">
      {/* Background glow */}
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between ml-1">
                ...
                <button type="button" className="text-xs font-bold text-[#00AAFF] hover:text-white transition-colors">Forgot password?</button>
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
